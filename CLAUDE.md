...
- **Opt-in output effort (model-gated).** A registry entry MAY carry `effort: 'low'|'medium'|'high'|'xhigh'`. Handlers add `output_config: { effort }` only when the effort is set AND the resolved model supports it (`supportsEffort` in `lib/anthropic.ts`): Fable 5, Opus 4.8/4.7/4.6/4.5, Sonnet 5/4.6. On any other model the worker omits it — `output_config.effort` returns a 400 on e.g. Haiku 4.5 / Sonnet 4.5. `chat.ts` gates on the per-call model; `messages.ts` gates on `claudePayload.model || entry.model` and only applies when the client hasn't already set `output_config`. The four onboarding propose/review JSON paths (`onboarding-batch`, `onboarding-batch-all`, `onboarding-review`, `onboarding-review-all`) use `low` so Sonnet 5 cannot spend their output budget on adaptive thinking; placement and unrelated prompts keep their existing defaults.
