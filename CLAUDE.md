...
- `npm run dev` — local dev server (`wrangler dev`, port 8787). Local config comes from `.dev.vars` — copy `.dev.vars.example` (its default `ANTHROPIC_API_BASE` already points at the free dev shim below).
