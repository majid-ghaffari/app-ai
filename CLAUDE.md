...
| [docs/TESTING.md](docs/TESTING.md)                   | The required offline test layers plus the optional live token-savings diagnostic                 |
...
- **Brain Is the Single Source of Truth** — anything crossing the Personalizer API boundary mirrors Brain's C# exactly; verify in the sibling `brain` repo, never the sibling `app` TS, which can be stale (Hard rules below).
- **Secrets stay server-side** — API keys, bearer tokens, cookies, passwords, and service credentials are never model-visible and are redacted by the logging runtime. The request context-ID is intentionally retained as a private operational correlation field (docs/TOOLSETS.md; fitness-tested).
...
│   ├── cache-control.ts  # manageCacheControl() — the 4-block prompt-cache limit + explicit stable user-prefix priority; EXTENDED_CACHE_CONTROL (1h TTL) const
...
│   ├── dev-inference-trace.ts # Correlated /messages + /chat model/tool trace routed at TRACE level
│   └── logger.ts         # Request-scoped structured logger; LOG_TARGETS_* fan-out to console/Seq
...
├── prompt-registry.ts    # System-prompt REGISTRY: name → { prompt, tier, maxTokens, usesTools, description, attachments }; reads text via the src/prompts/ barrel
├── prompts/              # The prompt TEXT layer (one location — pure .md, bundled at build time)
│   ├── index.ts          # Barrel: imports every prompt's .md (and assembles the composed prompts) and re-exports each prompt's FINAL text as a named export the registry reads
│   ├── _compose.ts       # composePrompt(...parts) — joins ordered block strings into one composed system prompt
│   ├── *.md              # SIMPLE prompt TEXT (one .md per prompt)
│   ├── <name>/           # MULTI-FILE prompt (folder): prompt.md (system) + sample.md (attachment)
│   └── <name>/index.ts   # COMPOSED prompt: its own barrel assembles the prompt from _-prefixed block fragments + onboarding-shared/ blocks via composePrompt
```
...
- **Extended 1h prompt cache (`EXTENDED_CACHE_CONTROL` in `cache-control.ts`).** The STABLE prefix uses `{ type: 'ephemeral', ttl: '1h' }` (GA — no beta header): `messages.ts` system block + reused file-attachment blocks; `chat.ts` `buildSystem` base prompt block (the volatile context block AND the Referenced-Entities block stay uncached — the system array is `[ registry (1h) , context? , referenced-entities? ]`, per-call blocks last so the cached prefix stays byte-stable); the placement file-prefix block (last `fileIds` block); and the user blocks `manageCacheControl` keeps. On `/messages`, a caller marks the terminal original-plus-round-0 stable reference block with `limespot_stable_cache_prefix: true`; cache management reserves its 1h breakpoint after counting the one/two system blocks and prompt attachments, strips the internal marker before both token counting and inference, preserves block/file-ID order, and ranks other user images/documents only for the remaining slots. Conversation turns use the 5-min default (short-lived).
...
- **Opt-in output effort (model-gated).** A registry entry MAY carry `effort: 'low'|'medium'|'high'|'xhigh'`. Handlers add `output_config: { effort }` only when the effort is set AND the resolved model supports it (`supportsEffort` in `lib/anthropic.ts`): Fable 5, Opus 4.8/4.7/4.6/4.5, Sonnet 5/4.6. On any other model the worker omits it — `output_config.effort` returns a 400 on e.g. Haiku 4.5 / Sonnet 4.5. `chat.ts` gates on the per-call model; `messages.ts` gates on `claudePayload.model || entry.model` and only applies when the client hasn't already set `output_config`. The per-page onboarding propose/review paths use `low`; the whole-store `onboarding-batch-all` / `onboarding-review-all` paths use `medium` for exhaustive multi-page reasoning and visual-action output. Placement and unrelated prompts keep their existing defaults.
...
### Prompt registry (`prompt-registry.ts` + `src/prompts/`)

`prompt-registry.ts` is a thin registry mapping prompt name → metadata:

```
{ prompt, model, maxTokens, usesTools, description, attachments, effort?, clientTools? }
```

(`effort` is optional — see Token-saving levers above. `clientTools` is optional and mutually exclusive with `usesTools` — see CLIENT-tool mode above; its first consumer is `onboarding-chat` (`look_at_page`, #111).)

It is the **single source of truth** for each prompt's model / token / tool choices — both `handlers/chat.ts` (agent loop) and `handlers/messages.ts` (system-prompt injection) read text + metadata through `getSystemPrompt(name, env)`. The model is chosen via a **two-layer indirection** (No Hardcoded Config Values): each registry entry carries a provider-agnostic capability **`ModelTier`** — `fast` (latency-sensitive / cheapest — the one-shot `placement` prompt), `balanced` (the multi-modal / JSON reasoners: `proposals`, `visual-verify`, the onboarding batch/review passes, the behavioral/visual/generative probes `probe-cart-wiring` / `probe-style` / `probe-template`), or `frontier` (the most-capable flagship, the **default** tier — the agent-loop chat prompts + `probe-industry`). An entry that omits `tier` gets the `frontier` default. The tier→model **binding** is the ONLY place a concrete model id appears: `wrangler.toml [vars]` (`MODEL_FAST` / `MODEL_BALANCED` / `MODEL_FRONTIER`), read via `resolveModel(env, tier)` when the entry is resolved. Bumping a model version OR switching provider (Claude → OpenAI / Gemini) is a config-only change — the registry and every prompt stay untouched. All prompt text lives ONLY under `src/prompts/` (there is exactly one prompt location — no second folder anywhere in the repo). The `src/prompts/` **barrel** (`index.ts`) imports each `.md`, re-exports every prompt's FINAL text as a named export, and (via `getSystemPrompt`) is where `resolveModel` maps the entry's tier → model. Each COMPOSED prompt is self-contained in its own folder's `index.ts`, which assembles its `_`-prefixed block fragments + `onboarding-shared/` blocks with `composePrompt` (`_compose.ts`) and default-exports the finished text the barrel re-exports. The registry just does `import * as prompts from './prompts'` (resolving to the barrel), so each registry entry's `prompt:` is `prompts.<name>`.

**Unified single / multi-file / COMPOSED convention.** A prompt is one of three shapes; all resolve to the same registry entry:
...
- **Multi-file** → a folder: `src/prompts/<name>/` (e.g. `image-selection/`), made of parts:
  - `prompt.md` — the main system prompt (re-exported by the barrel).
  - a sample/attachment file (e.g. `sample.md`) — NOT part of the system prompt; re-exported by the barrel and wired as an `attachments` entry `{ content, mimeType, filename }` so `messages.ts` uploads it (Files-API dedup + extended cache) and prepends it to the first message as a document block. Attachments are image-selection-only today.
- **Composed** → a folder of `_`-prefixed BLOCK files, no `prompt.md`, PLUS its own `index.ts` that assembles the system text with `composePrompt(...)` (the helper lives in `src/prompts/_compose.ts`) from an ORDERED mix of per-prompt fragment files in the prompt's own folder (`_intro.md`, `_receive.md`, …) plus SHARED blocks under `src/prompts/onboarding-shared/` (`_shared-*.md` / `_block-*.md` — box vocabulary, the JSON-only hardening lines, the mobile rule, the correction verbs, …) so every composing prompt shares that instruction (edit a block once → all of them update). The composed prompts are the onboarding propose/review pairs (`onboarding-batch` / `-all`, `onboarding-review` / `-all`), the cart-drawer conductors (`cartdrawer-batch-all`, `cartdrawer-review-all`), and `optimize-demand`; the full authoritative inventory — which folders are prompts vs shared blocks, and why the `-all` twins are not duplicates — is in [src/prompts/CLAUDE.md](src/prompts/CLAUDE.md). Here a `_`-prefixed `.md` is a BLOCK PART that IS explicitly imported + composed — it just isn't a standalone `<name>` you select via the header. The composed text is pinned byte-for-byte by the byte-equivalence gate (`test/onboarding-prompt-blocks.test.ts`) and its STABLE ORDER is the cacheable system prefix (the `cache_control` 1h breakpoint in `messages.ts`; per-shop screenshots + manifest ride AFTER it — "variable data last"). Per-prompt design/maintenance docs live at `docs/prompts/<name>.md` (process: [docs/PROMPT-AUTHORING.md](docs/PROMPT-AUTHORING.md)).

`image-selection` is the multi-file prompt with an attachment (system text `prompt.md` + the `sample.md` attachment); the composed prompts are the onboarding + cart-drawer + optimize set. Design docs: [docs/prompts/image-selection.md](docs/prompts/image-selection.md), [docs/prompts/onboarding-batch.md](docs/prompts/onboarding-batch.md), [docs/prompts/onboarding-review.md](docs/prompts/onboarding-review.md).

The whole-store proposal and review prompts own currency as part of their shared visual-action union.
If the initial proposal omits a valid verdict, the registered `onboarding-currency-recovery` one-shot
reuses at most four already-uploaded price tiles and returns exactly one candidate selection or strictly
validated six-argument format before the first QC render. It is a bounded omission recovery, not a
post-QC currency island; its maintenance contract is
[docs/prompts/onboarding-currency-recovery.md](docs/prompts/onboarding-currency-recovery.md).

An ILLUSTRATIVE subset of the registered prompts follows; the COMPLETE inventory — every registered prompt with its shape, endpoint, tools, and role — is the authoritative table in [src/prompts/CLAUDE.md](src/prompts/CLAUDE.md), which also owns the per-prompt doc-sync rule:
...
| `proposals`          | `/chat`     | Yes   | Per-store onboarding SETUP proposer; reasons over REAL PAGE EVIDENCE (cleaned HTML + screenshots, uploaded via `/files` and passed as `fileIds`) AND pulls the merchant's REAL store data on demand via the personalizer toolset (`get_store_analytics` / `list_segments` / `list_campaigns` / `get_store_config`) through the `/chat` tool-use loop, returns a structured JSON proposal (boxes/segments/progress-bar/bundles). The AOV-derived progress-bar threshold comes from real analytics. Tier: `balanced` (currently `claude-sonnet-5`). Drives lib `ai/brain/proposals.ts`. |
...
| `probe-industry`     | `/messages` | No    | Website-Analysis PROBE: given a homepage screenshot + cleaned HTML (uploaded via `/files`, like image-selection), returns JSON-only `{ industry, confidence, findings }`. No tools. Tier: `frontier` / default (currently `claude-opus-4-8`).                                                                                                                                                                                                                                                                                                                                         |
| `probe-cart-wiring`  | `/messages` | No    | Website-Analysis PROBE (`balanced` tier, currently `claude-sonnet-5`): classifies cart UI from BEHAVIORAL evidence (closed → opened → populated screenshots + cleaned HTML + driver notes), returns JSON-only selectors + `surfaceType` (`drawer`/`modal`/`dropdown`/`native`) + `refreshAuto`. No tools; never invents a selector (null when unsure).                                                                                                                                                                                                                                |
| `probe-style`        | `/messages` | No    | Website-Analysis PROBE (`balanced` tier, currently `claude-sonnet-5`): observes EXISTING product-card styling (layout, typography, price, saleSign, cta, arrows, image) from cleaned HTML + product-page screenshots, returns a JSON-only style descriptor. No tools; reads colours/fonts verbatim from the site CSS and omits unknowns.                                                                                                                                                                                                                                              |

### Website-Analysis probes (`probe-<id>`)

A **probe** is a header-selected `POST /messages` system prompt for Website Analysis: it takes storefront evidence (cleaned-HTML documents + screenshot images, uploaded via `POST /files` and referenced as content blocks on the first user message — the same muscle as `image-selection`) and returns **JSON ONLY** per a fixed schema (no tools, no prose). `/messages` needs no per-probe change — it serves any `X-Personalizer-System-Prompt`-selected prompt. The convention lives in `prompt-registry.ts`: every probe registers under a `probe-<id>` name (`PROBE_PREFIX = 'probe-'`, `isProbeName()`), ships its system text at `src/prompts/probe-<id>/prompt.md`, records its frozen output contract in the `PROBE_SCHEMAS` index, and has a maintenance doc at `docs/prompts/probe-<id>.md`. A new probe drops in identically — a new prompt file + registry entry + `PROBE_SCHEMAS` entry + maintenance doc, no handler edit. The registered probes: `probe-industry` (homepage → `{ industry, confidence, findings }`, `frontier` tier / default); `probe-cart-wiring` (behavioral cart classification → selectors + `surfaceType` + `refreshAuto`, `balanced` tier); `probe-style` (existing product-card styling → style descriptor, `balanced` tier); `probe-template` (screenshot + HTML → a generated rec-box card-slot template `{ html, css, cssScoped, baseTemplateName, confidence, notes }`, `balanced` tier). `probe-industry`'s frozen contract is `POST {base}/messages` with `X-Personalizer-System-Prompt: probe-industry`, body = a standard Anthropic messages payload whose first user content is `[document(cleaned-HTML file_id), image(screenshot file_id)]`, response = the Anthropic envelope with assistant text `{ "industry": string, "confidence": number (0..1), "findings": string[] }`.
...
To add/edit a prompt: create/edit the `.md` file(s) under `src/prompts/` (a single `.md` for a simple prompt, or a `<name>/` folder for a multi-file prompt), add/update its registry entry in `prompt-registry.ts`. If a prompt needs runtime `${}` interpolation it can't be a static `.md` — keep that dynamic bit in JS.
...
Latency-critical visual `/messages` prompts (onboarding proposal/review, currency recovery,
cart-drawer, optimize, and visual verification) proceed directly to inference instead of awaiting the
advisory `count_tokens` diagnostic. Other large payloads may still use that best-effort estimate; it
never gates correctness.

Application code logs only through the request-scoped `LoggingRuntime` in `lib/logger.ts`. Each
`LOG_TARGETS_<LEVEL>` binding routes that level to `ignore`, `console`, `seq`, or a comma-separated
fan-out; environment names carry no policy. Production routes `error`/`fatal` to private Seq and all
other levels to `ignore`, with Cloudflare application console output disabled. Local configuration
routes every level to both console and Seq. Seq delivery uses `waitUntil`, is fail-open, and accepts
an optional `LOG_SEQ_API_KEY`; the configured shared endpoint supports anonymous CLEF ingestion.

`/messages` and `/chat` emit correlated TRACE records for complete model-visible input, assistant
output, tool execution, usage, status, and elapsed time. File IDs and request context IDs are retained
for correlation. The logger preserves stacks, causes, structured
provider/tool details, and redacts actual secret-bearing fields. `npm run dev` mirrors local console
output to `app-ai-dev.log`; routing TRACE to `ignore` disables inference traces without a code branch.

## Commands

- `npm run dev` — local dev server (`wrangler dev`, port 8787) via the `scripts/dev.mjs` launcher, which resolves `git rev-parse --short HEAD` IN NODE (shell-independent) and passes it as `--define __APP_AI_REV__:"<sha>"` so `/health` reports the REAL served rev as `buildRev` (not `'dev'`). It FAILS LOUD (non-zero exit, never a silent no-define) if git can't resolve a revision, so a stale `buildRev:"dev"` worker can't start green. The launcher mirrors Wrangler stdout/stderr to `app-ai-dev.log` (`APP_AI_DEV_LOG_FILE` overrides the path), making local `[InferenceTrace]` JSON events readable by another process. Local config comes from `.dev.vars` — copy `.dev.vars.example` (its default `ANTHROPIC_API_BASE` already points at the free dev shim below).
- `npm run dev:shim` — the **free dev LLM channel** (`scripts/dev-claude-shim.mjs`, port 8788). Start it, then plain `npm run dev` fulfils every inference via Claude Code — no Anthropic credits. See "Local AI dev — the free Claude-Code channel" below.
...
## Local AI dev — the free Claude-Code channel (default)

Local dev fulfils Anthropic inference through **Claude Code**, not the paid API, so dev never burns Anthropic credits. This is the standard, permanent local channel.

**How it works.** The worker talks to Anthropic through exactly ONE seam — `src/lib/anthropic.ts`, where every call is `fetch(`${ANTHROPIC_API_BASE}/v1/...`)` and `ANTHROPIC_API_BASE` is read via `anthropicApiBase(env)`. So the whole toggle is one line in `.dev.vars`:

```
ANTHROPIC_API_BASE=http://127.0.0.1:8788
```

`scripts/dev-claude-shim.mjs` (started by `npm run dev:shim`) is a local Anthropic-API-compatible HTTP server on `:8788` that fulfils each request via the `@anthropic-ai/claude-agent-sdk` `query()` — using the machine's Claude Code auth. No `src/` change, no `[env.dev]` block: plain `wrangler dev` reads `.dev.vars`, which overrides the production `[vars]` (`https://api.anthropic.com`). Production is unchanged.

**Run it (two terminals, or shim in the background):**

```
npm run dev:shim     # terminal 1 — the free Claude-Code shim on :8788
npm run dev          # terminal 2 — wrangler dev on :8787, ANTHROPIC_API_BASE → the shim
```

**The FREE-stack worker MUST be started via `npm run dev` (the `scripts/dev.mjs` launcher).** It stamps the served worker with `--define __APP_AI_REV__:"<short HEAD>"`, so `/health` reports the exact rev as `buildRev`. The lib's FREE-stack preflight (`preflightMatrixStack` in `tests/e2e-playwright/studio-ai/fixtures/harness-boot.ts`) always rejects a `'dev'` / absent `buildRev`, AND — under `LS_EXACT_STACK` (the fork-matrix evidence run) — the exact-revision compare fires by default (GAP-A): it compares the served `buildRev` against an EXPECTED rev that **DEFAULTS to the sibling `../app-ai` checkout's frozen HEAD** (`git -C ../app-ai rev-parse --short HEAD`) when `LS_APP_AI_EXPECTED_REV` is unset, so a stale worker at a real short-sha FAILS CLOSED instead of passing green (the env pin overrides the default; `LS_APP_AI_REPO_PATH` points elsewhere; an unresolvable checkout is a hard stop in exact-stack mode). The auto-resolve default is GATED on `LS_EXACT_STACK` — symmetric with the lib asset-hash pin — so a NON-exact-stack caller that doesn't pin the rev stays lenient (a worker legitimately serving an earlier commit than a since-pulled `../app-ai` HEAD is not spuriously rejected). A stale worker would exercise code other than the reviewed checkpoint. See `preflight-fail-closed.spec.ts`. A bare `wrangler dev`, a non-shell supervisor, or `wrangler deploy` yields `buildRev:"dev"` and is NOT valid evidence. Before any evidence run, verify the served rev matches HEAD:

```
test "$(curl -s 127.0.0.1:8787/health | jq -r .buildRev)" = "$(git rev-parse --short HEAD)"
```

If it differs, the worker was launched without the define — restart it via `npm run dev`. The deploy path (`wrangler deploy`) stays define-less by design — production is not the exact-marker FREE stack (`src/build-rev.ts`).

**What the shim implements** (exactly the surface the worker calls):

- `POST /v1/messages` non-stream — the probes / `createMessage` path. Accepts content blocks incl. `image` + `document` (`source:{type:'file',file_id}`) + a `system` array; translates to a `query()` call; returns one Anthropic body `{ content:[{type:'text',text}], usage, stop_reason }`. A wrapping `code fence` is stripped so JSON-only probes/proposals parse cleanly.
- `POST /v1/messages` `stream:true` SSE — the `/chat` `runAgentLoop` path. Maps the SDK's assistant text to `text_delta` frames and synthesizes the full Anthropic envelope (`message_start` / `content_block_start` / `content_block_delta` / `content_block_stop` / `message_delta{stop_reason}` / `message_stop`) that `src/handlers/chat.ts` parses.
- `POST /v1/files`, `GET /v1/files/{id}`, `DELETE /v1/files/{id}` — an in-memory temp store keyed by a generated `file_id`, remembering mime (image vs document); rehydrated into the `query()` user turn as inline base64 at message time.
- `POST /v1/messages/count_tokens` — `{ input_tokens }` (char/4 estimate; advisory).
- **Tool-use bridge** for the `/chat` tool loop: each personalizer tool (`get_store_analytics` / `list_segments` / `list_campaigns` / `get_store_config`) is registered as an in-process `createSdkMcpServer()` tool whose handler makes the SAME Personalizer `v2/ai-tools/*` call the worker's personalizer toolset makes. The SDK resolves the loop; the shim emits the equivalent `tool_use` frames (bare tool name, harness built-ins filtered out). Enable it by setting `LS_DEV_CONTEXT_ID` (a dev store's context-ID, e.g. majiddev-4) — the worker's Anthropic seam does not forward the context-ID to Anthropic, so the shim needs it out of band. Without it, tool flows run tool-free (fall back to the real API only if you deliberately switch channels).

**Using the real, paid API from local (rarely — prod-only by policy)** requires TWO deliberate steps: set `ANTHROPIC_API_BASE=https://api.anthropic.com` AND `AI_CHANNEL=prod`. The **cred-burn guard** (`test/dev-shim.test.ts` → `assertDevAiChannel`) FAILS any dev/test config that points at `api.anthropic.com` without `AI_CHANNEL=prod`, so a credit-spending default can never slip in silently. `scripts/dev-claude-shim.mjs` and its guard/translation layer are dev-only — never bundled, never run in production.
...
- `test/proxy.test.ts` — the proxy surface: CORS, response/error shape, context validation, the Anthropic client (headers/beta/URLs, config-error rejection), `manageCacheControl` (legacy ranking plus explicit stable-prefix priority across one/two system blocks and attachments, the four-slot cap, marker stripping/order preservation), and the `/files` + `/messages` + `/health` handlers.
- `test/probes.test.ts` — the Website-Analysis probe mechanism: the registry resolves `probe-industry` (text/metadata/model), resolves the Sonnet probes `probe-cart-wiring` + `probe-style` on the `balanced` tier (≠ the default `frontier` tier, `usesTools: false`, no attachments) with their selector/style-descriptor prompt contracts, the `probe-<id>` convention (`PROBE_PREFIX` / `isProbeName`) + the `PROBE_SCHEMAS` index ↔ registry coherence, `/messages` routing the `probe-industry` / `probe-cart-wiring` / `probe-style` headers (system block injected, no attachments, model defaulted to the registry entry, caller blocks pass through), and the frozen `{ industry, confidence, findings }` output contract (schema/parse checks: well-formed, `Unknown`/empty-findings sink, rejects missing keys / out-of-range confidence / non-string findings).
- `test/proposals.test.ts` — the TOOL-CAPABLE per-store SETUP PROPOSALS prompt: the registry resolves `proposals` on the `balanced` tier (Sonnet, ≠ the default `frontier` tier) + `usesTools: true`, the prompt reasons over page evidence (screenshots + cleaned HTML) AND instructs the model to pull real store data via the personalizer tools (AOV-derived progress-bar threshold), and `POST /chat` with the `proposals` header sends the personalizer tool surface + the page-evidence file blocks and drives the tool loop (tool_call → Brain `v2/ai-tools` → tool_result → final JSON proposal).
- `test/config.test.ts` — `src/config.ts`: the no-fallback contract (every accessor throws on a missing/empty variable, naming it and where to set it), the `resolveModel(env, tier)` tier→model mapping (each tier → its bound var, throws named when absent) + the PRODUCTION `[vars]` pins read from wrangler.toml itself (`PERSONALIZER_API_URL = https://personalizer.io`, the Anthropic origin, the tier→model bindings `MODEL_FAST = claude-haiku-4-5` / `MODEL_BALANCED = claude-sonnet-5` / `MODEL_FRONTIER = claude-opus-4-8`, the credentialed-origin allowlist, and no secret in the file).
- `test/architecture.test.ts` — fitness scans over `src/` + `scripts/` enforcing the standing invariants: explicit endpoint toolset allowlists; credentials never in logs or the model-visible system builder; one error shape (no bespoke `{ error: { message } }` envelopes); No Hardcoded Config Values (no absolute URL literals in src or scripts, no inline `||`/`??` fallbacks on `env`/`process.env` reads, required worker vars read only through `src/config.ts`). The dev Claude-Code shim (`scripts/dev-claude-shim.mjs`) is carved out of the no-URL-literal scan only — it is standalone local tooling with overridable dev-default endpoints, outside the worker's fail-fast config contract (never bundled); it still passes the env-fallback scan (it reads config via `envOr(name, fallback)` bracket access, not inline `||`/`??`).
- `test/dev-shim.test.ts` — the free dev Claude-Code channel (`scripts/dev-claude-shim.mjs`): the pure Anthropic⇄Claude-Code translation layer (`systemToString`, `rehydrateBlock` file→inline base64 image/document + tool_result flatten + unknown-file degrade, `normalizeContent`, `buildPromptMessages` user turn + assistant-history preamble, `estimateTokens` char/4, `bareToolName` mcp-prefix strip, `stripCodeFence`, `usageFrom`, `extractFilePart` multipart), plus the **cred-burn GUARD** (`assertDevAiChannel`: throws when a dev/test run points `ANTHROPIC_API_BASE` at `api.anthropic.com` without `AI_CHANNEL=prod`; allows the local shim; opt-in with `AI_CHANNEL=prod`) and a check that `.dev.vars.example` ships the free-shim default.
- `test/dev-inference-trace.test.ts` + `test/logger.test.ts` — correlated inference events, complete diagnostic text/tool/file decisions, per-level multi-sink routing, context/error detail, secret-field redaction, optional Seq authentication, fail-open delivery, and background rejection capture.
...
An optional **live token-savings diagnostic** (`scripts/verify-caching.mjs`, `npm run verify:caching`) runs against the DEPLOYED worker. It needs a master context-ID + a reachable target, so it is OUT of `vitest run`. When explicitly invoked, it asserts `cache_read_input_tokens > 0` on a repeat call for the chat tools+system prefix and the placement file-block prefix, and throws loudly (never silently skips) on a 0-read regression or missing creds. It is not a release gate. See [docs/TESTING.md](docs/TESTING.md).
...
- **Caching diagnostic workflow** (`.github/workflows/verify-caching.yml`) is an optional `workflow_dispatch` job guarded on the `APP_AI_CONTEXT_ID` + `APP_AI_TARGET` repo secrets. The guard step exits cleanly when either secret is absent, so it never fails a fork or unconfigured repo, and it is never wired into the required push/PR `build` job. It may also be run manually with `npm run verify:caching` and explicit `APP_AI_TARGET` + `APP_AI_CONTEXT_ID`/`APP_AI_CONTEXT_FILE`; the script takes no defaults.
...
- **No AI-attribution in commit messages.** A commit message contains NO `Co-Authored-By: Claude …` / `Claude-Session:` trailer, no "Generated with …" line, and no mention of Claude / Anthropic / any AI assistant as an author or contributor — the subject + body describe the change, nothing else. Mirrored in [CONTRIBUTING.md](CONTRIBUTING.md) → Commit message convention.
