...
- **`addBox`** — add a box the plan should have included, OR re-place one that landed in the wrong slot (as a `removeBox` + `addBox` pair). `args`: `{ "page": "<page>", "boxType": "<box>", "position": "before" | "after" | "replace", "anchorNumber": <int>, "appearancePatch": { ... } | null, "appearancePatchMobile": { "ImageHeightMobile"?: <int>, "MarginRightMobile"?: <int> } | null }`. `position` + `anchorNumber` + the two patches use the SAME semantics as the propose step: `anchorNumber` is a numbered SECTION the page already has (a `1..N` marker the propose step chose from), `position` is whether the box goes before it, after it, or replaces it (`replace` only for a clear swap target), and `appearancePatchMobile` is the optional mobile-only override. Never invent a location or emit a selector — just the number + before/after/replace.
