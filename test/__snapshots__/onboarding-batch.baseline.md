...
12. **`progressBar`** (OPTIONAL, Cart page ONLY) — omit it entirely unless `page` is `Cart`. On Cart, add it when a threshold nudge fits: `{ "reasoning" }` — just one short `reasoning`. The bar always renders at the TOP of the Cart page, so there is NO `position` / `anchorNumber` to choose and NO appearance keys (the campaign template styles the bar). Omit the key (or set it `null`) if the Cart already shows a threshold bar or one wouldn't help.
