...
describe('optimize-demand — demand-first, composed in the FROZEN cache-first order', () => {
  ...
  it('PINS the empty-means-empty contract — a zero-length `boxes` array, NEVER a placeholder object', () => {
    const text = composed();
    // The zero-proposal branch: when no box is warranted the ENTIRE reply is a
    // literal empty array. The live OPTIMIZE oracle requires this exact-empty
    // shape (target page echo + `[]`), so the prompt MUST teach it explicitly.
    expect(text).toContain('"boxes": []');
    // The never-placeholder half — an array of empty/partial objects is malformed
    // output, NOT "no boxes". Pin the prohibition so the model can't emit `[{}]`.
    expect(text).toMatch(/zero-length array/i);
    expect(text).toContain('"boxes": [{}]');
    expect(text).toMatch(/malformed output/i);
  });
});
