...
Return EXACTLY one JSON object with a top-level key `boxes` — an array of the box(es) to place INSIDE the drawer — plus an OPTIONAL `progressBar` key. There is NO `pages` wrapper and NO per-page map: this is ONE surface (the open drawer), so the surface is IMPLICIT and every box in `boxes` belongs to the drawer. The OPTIONAL `progressBar` is the Smart Progress Bar's SECOND host (the playbook places one on the Cart page AND, when the store uses a sliding drawer, at the top of the drawer): `{ position, anchorNumber, reasoning }` — the same numbered-candidate language as a box, canonically at the very TOP of the drawer (`"before"` the first drawer section) so the shopper sees the threshold nudge above the line items. Omit the key when the drawer already shows a visible threshold / progress bar, or when a bar would not help. There are NO off-page `segments` / `discounts` keys (those are store-wide, decided by onboarding, not here). A drawer that genuinely warrants no box gets `{ "boxes": [] }`. Nothing else.
...
2. **Exact schema.** One top-level key `boxes` — an array — plus the OPTIONAL `progressBar`. NO `pages` wrapper, NO `segments` / `discounts`. Each box has `boxType`, `position`, `anchorNumber`, `appearancePatch`, `appearancePatchMobile`, `reasoning`, and the OPTIONAL `styleReferenceSelector`. The `progressBar`, when present, is `{ position, anchorNumber, reasoning }` — NO appearance keys (the bar's look comes from its campaign template) and a `position` of `"before"` / `"after"` a numbered candidate (never `"replace"` — a bar augments the drawer, it never swaps a section out). The surface is implicit (the drawer) — a box carries no `page` field.
...
4. **`position`** — exactly `"before"` or `"after"`. Placement is ADDITIVE ONLY: never emit `"replace"` for any number — the drawer's own content all stays.
5. **`anchorNumber`** — an INTEGER in the drawer's `1..N` range, equal to a numbered candidate you actually SEE on the drawer's images . Numbering is one continuous sequence shared across both widths. Together with `position` it is ONE responsive placement for both widths. If no candidate fits a box you wanted, drop that box rather than inventing a place. Emit only the number + `before`/`after` — never a selector.
6. **`styleReferenceSelector`** (OPTIONAL, PREFERRED) — a CSS selector for the store's own most-representative product block to CLONE from (the drawer's own line-item cards, or a product grid / carousel). A green reference candidate's manifest `selector` is ideal (its `outerHTML` grounds the clone). Include it whenever a good reference exists; omit it (and lean on `appearancePatch`) only when there is none. Emit a plain selector string, never a value for a block you don't see.
