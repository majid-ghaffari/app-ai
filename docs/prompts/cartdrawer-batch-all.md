...
The assistant text is a single JSON object with a SINGLE top-level key `boxes`. **There is NO `pages` wrapper and NO per-page map** — this is ONE surface (the open drawer), so the surface is IMPLICIT and every box in `boxes` belongs to the drawer. **There is NO `progressBar` key** (the Smart Progress Bar is a Cart-PAGE item, not a drawer item — the drawer is too compact) and **NO off-page `segments` / `discounts` keys** (those are store-wide, decided by onboarding). A drawer that genuinely warrants no box returns `{ "boxes": [] }`.
