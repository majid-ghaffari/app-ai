...
- `appearancePatch` — `null`, OR an object of DESKTOP-and-shared keys using ONLY: `Style` (`"carousel"`|`"grid"`|`"bundle"`|`"rows"`), `ItemsPerPage`, `ItemsLimit`, `ImageBorderRadius`, `NavigationArrowType`. The FALLBACK when no `styleReferenceSelector` reference exists. Chosen as a compromise that reads well at BOTH widths.
