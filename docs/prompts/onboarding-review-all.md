...
```json
{
  "pages": {
    ...
    "Product": {
      ...
      "corrections": [
        { "action": "styleBox" | "removeBox" | "addBox" | "moveBox", "args": { "page": "Product", ... } }
      ],
...
- Each page's verdict is exactly `{ pass, feedback, corrections, failureClass }` — the identical per-page contract as [`onboarding-review`](onboarding-review.md):
  ...
  - `corrections` — array of `{ action, args }`, **limited to the four existing write verbs** (`styleBox` / `removeBox` / `addBox` / `moveBox`), each `args.page` echoing that page so the conductor targets it. `styleBox` `args`: `{ page, boxType, appearancePatch?, appearancePatchMobile? }` (`appearancePatchMobile` = the two mobile knobs `ImageHeightMobile` / `MarginRightMobile`, to fix a MOBILE-ONLY break without touching desktop); `removeBox` `args`: `{ page, boxType }`; `addBox` `args`: `{ page, boxType, position: "before"|"after", anchorNumber, appearancePatch|null, appearancePatchMobile|null }` (additive-only — never `replace`; only for a box the page does NOT already have); `moveBox` `args`: `{ page, boxType, position: "before"|"after", anchorNumber }` (no appearance keys) RE-PLACES a box the page ALREADY has. A re-place is ONE `moveBox`, never a `removeBox` + `addBox` pair: the lib's `guardReviewCorrections` drops an `addBox` for a box already in the applied plan and treats add-after-remove on the same box as a reversal, so the pair would delete the box instead of moving it. Empty `[]` on pass, or when a real problem has no safe correction.
